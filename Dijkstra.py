...
nodes = {
    ...
    'Vélingara' : {'Médina Yoro Foulah' : 92.8,
                   'Kolda' : 102},
...
def dijkstra(dep, arr):
    itineraire_1 = []
    # Initialisation
    #s_dep = {dep : [0, [dep]]}          # Sommet initialise à 0
    #autre_s = {k : [math.inf, ''] for k in nodes if k != dep}       # Autre sommet à l'infini
    itineraire = []
    itineraire.append(dep)
    distance =  0 #s_dep[dep][0]
    actual_nodes = {}
    while itineraire[-1] != arr:
        itineraire.append(arr)
        poids = []
        for i in nodes:
            if i == dep:
                actual_nodes = i
